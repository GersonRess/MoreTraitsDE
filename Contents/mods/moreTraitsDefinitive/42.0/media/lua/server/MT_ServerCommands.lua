    ...
    for _, index in ipairs(PartIndexes) do
        ...
        if bodyPart then
            ...
            if args.indefatigable then
                if args.skipRestoreList and MT.TableContains(args.skipRestoreList, index) then
...
local function ProcessGlassBody(player, args)
    ...
    if args.damage ~= nil then
        bodyDamage:ReduceGeneralHealth(args.damage)
    end
...
local function ProcessProwessGuns(player, args)
    if not args.weaponID then
        return
    end

    local primaryWeapon = player:getPrimaryHandItem()
    if not primaryWeapon or primaryWeapon:getID() ~= args.weaponID then
        return
    end

    primaryWeapon:setCurrentAmmoCount(primaryWeapon:getCurrentAmmoCount() + 1)
end
